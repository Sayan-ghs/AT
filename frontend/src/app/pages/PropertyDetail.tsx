...
import { TrendingUp, MapPin, Building2, Coins, Loader2 } from "lucide-react";
...
export function PropertyDetail() {
  ...
  const assetId      = gs ? Number(gs.assetId) : 0;
  ...
  const available    = totalTokens - tokensSold;
  const soldPct      = totalTokens > 0 ? Math.round((tokensSold / totalTokens) * 100) : 0;
  const availPct     = 100 - soldPct;

  const propertyName    = (import.meta.env.VITE_PROPERTY_NAME as string)        || "Real Estate Token";
  const propertyLoc     = (import.meta.env.VITE_PROPERTY_LOCATION as string)    || "Algorand Testnet";
  const propertyYield   = parseFloat((import.meta.env.VITE_PROPERTY_YIELD as string) || "0");
  const propertyDesc    = (import.meta.env.VITE_PROPERTY_DESCRIPTION as string) || "Tokenized real estate property secured on the Algorand blockchain.";

  const ownershipData = [
    { name: "Available", value: available || 1, color: "#E5E5E5" },
    { name: "Sold",      value: tokensSold,     color: "#00FF88" },
  ];
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-[1440px] mx-auto px-8 py-12">
          <div className="flex flex-col items-center justify-center h-64 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <p className="text-2xl font-bold uppercase">No Asset Created</p>
            <p className="text-gray-500 mt-2 font-bold">Admin has not created a property asset yet.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-12 gap-8 mb-12">
          ...
          <div className="col-span-7">
            <div className="border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden aspect-[4/3]">
              <img src={PROPERTY_IMAGE} alt={propertyName} className="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <div className="col-span-5">
            <div className="bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] sticky top-32">
              ...
              <div className="flex items-center gap-2 text-gray-400 mb-2">
                ...
                <span className="font-bold uppercase">{propertyLoc}</span>
              </div>
              <p className="text-xs font-mono text-gray-400 mb-6">ASA ID: {assetId}</p>
              ...
              <div className="space-y-6 mb-8">
                <div className="flex justify-between items-center pb-4 border-b-3 border-black">
                  ...
                  <span className="text-3xl font-mono font-bold">{priceAlgo} ALGO</span>
                </div>
                <div className="flex justify-between items-center pb-4 border-b-3 border-black">
                  <span className="font-bold uppercase tracking-wide">Available Tokens</span>
                  <span className="text-3xl font-mono font-bold">{available.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center pb-4 border-b-3 border-black">
                  <span className="font-bold uppercase tracking-wide">Total Supply</span>
                  <span className="text-3xl font-mono font-bold">{totalTokens.toLocaleString()}</span>
                </div>
                {propertyYield > 0 && (
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-3xl font-mono font-bold text-[#00FF88]">{propertyYield}%</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-12">
          <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={ownershipData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value" strokeWidth={3} stroke="#000000">
            ...
            <div className="flex justify-center gap-8 mt-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold">Sold ({soldPct}%)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-gray-200 border-2 border-black" />
                <span className="font-bold">Available ({availPct}%)</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={[
                { month: "Jan", value: 6.2 }, { month: "Feb", value: 6.8 }, { month: "Mar", value: 7.1 },
                { month: "Apr", value: 7.5 }, { month: "May", value: 7.9 }, { month: "Jun", value: 8.2 },
                { month: "Jul", value: 8.5 }, { month: "Aug", value: 8.9 }, { month: "Sep", value: 9.2 },
                { month: "Oct", value: 9.5 }, { month: "Nov", value: 9.8 }, { month: "Dec", value: 10.2 },
              ]}>
                ...
                <Tooltip contentStyle={{ border: "3px solid #000000", backgroundColor: "#FFFFFF", fontWeight: "bold" }} />
                <Line type="monotone" dataKey="value" stroke="#00FF88" strokeWidth={4} dot={{ fill: "#000000", r: 6, strokeWidth: 3, stroke: "#000000" }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {tabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab.toLowerCase())}
                className={`flex-1 py-4 px-6 font-bold uppercase tracking-wide transition-colors border-r-4 last:border-r-0 border-black ${
                  activeTab === tab.toLowerCase() ? "bg-black text-white" : "bg-white text-black hover:bg-gray-100"
                }`}>{tab}</button>
            ))}
          </div>
          ...
          <div className="p-8 bg-white">
            {activeTab === "overview" && (
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-lg text-gray-600 leading-relaxed">{propertyDesc}</p>
                </div>
                <div className="grid grid-cols-3 gap-8">
                  <div className="border-4 border-black p-6">
                    <Building2 className="w-8 h-8 mb-4" />
                    <p className="text-sm font-bold text-gray-400 mb-2">ASA ID</p>
                    <p className="text-xl font-bold font-mono">{assetId}</p>
                  </div>
                  <div className="border-4 border-black p-6">
                    <Building2 className="w-8 h-8 mb-4" />
                    <p className="text-sm font-bold text-gray-400 mb-2">TOTAL SUPPLY</p>
                    <p className="text-2xl font-bold">{totalTokens.toLocaleString()}</p>
                  </div>
                  <div className="border-4 border-black p-6">
                    <Building2 className="w-8 h-8 mb-4" />
                    <p className="text-sm font-bold text-gray-400 mb-2">TOKENS SOLD</p>
                    <p className="text-2xl font-bold">{tokensSold.toLocaleString()}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between p-4 border-3 border-black">
                    ...
                    <span className="font-mono font-bold">{priceAlgo} ALGO</span>
                  </div>
                  <div className="flex justify-between p-4 border-3 border-black">
                    <span className="font-bold">Tokens Available</span>
                    <span className="font-mono font-bold">{available.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between p-4 border-3 border-black">
                    <span className="font-bold">Tokens Sold</span>
                    <span className="font-mono font-bold">{tokensSold.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between p-4 border-3 border-black bg-[#00FF88]">
                    <span className="font-bold">Market Cap</span>
                    <span className="font-mono font-bold">{(priceAlgo * totalTokens).toLocaleString()} ALGO</span>
                  </div>
                </div>
              </div>
            )}
            {activeTab === "governance" && (
              <div className="space-y-6">
                ...
                <div className="border-4 border-black p-6">
                  <p className="font-bold text-xl mb-2">Governance Coming Soon</p>
                  <p className="text-gray-600">On-chain governance for verified token holders will be available soon.</p>
                </div>
              </div>
            )}
            {activeTab === "documents" && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {["Property Deed", "Insurance Policy", "Inspection Report", "Token Contract"].map((doc) => (
                    <div key={doc} className="flex justify-between items-center p-4 border-3 border-black hover:bg-gray-100 cursor-pointer">
      ...
      <BuyModal
        ...
        property={{ name: propertyName, tokenPrice: priceAlgo, availableTokens: available }}
        assetId={assetId}
        onSuccess={() => fetchGlobalState().then(setGs)}
      />
    </div>
  );
}
